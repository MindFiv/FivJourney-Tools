...
class TestUserProfile:
    ...
    def test_update_user_profile_success(self, client: TestClient, auth_headers: dict):
        """测试更新用户资料成功"""
        update_data = {"full_name": "更新的全名", "phone": "13900000000", "bio": "更新的个人简介"}
    ...
    def test_update_user_profile_partial(self, client: TestClient, auth_headers: dict):
        """测试部分更新用户资料"""
    ...
    def test_update_user_profile_readonly_fields(self, client: TestClient, auth_headers: dict, test_user: User):
        """测试更新只读字段（应该被忽略）"""
...
class TestUserValidation:
    ...
    def test_update_invalid_phone(self, client: TestClient, auth_headers: dict, phone: str):
        """测试更新无效电话号码"""
    ...
    def test_update_empty_full_name(self, client: TestClient, auth_headers: dict):
        """测试更新空的全名"""
...
class TestUserQueries:
    ...
    def test_get_user_by_id_success(self, client: TestClient, auth_headers: dict, test_user: User):
        """测试通过ID获取用户信息成功"""
    ...
    def test_get_user_by_id_unauthorized(self, client: TestClient, test_user: User):
        """测试未认证获取用户信息"""
...
class TestUserSecurity:
    ...
    def test_user_response_no_sensitive_data(self, client: TestClient, auth_headers: dict):
        """测试用户响应不包含敏感数据"""
        response = client.get("/api/v1/users/me", headers=auth_headers)
    ...
    def test_inactive_user_cannot_access_profile(self, client: TestClient, test_inactive_user: User):
        """测试非活跃用户无法访问资料"""
...
class TestUserIntegration:
    ...
    def test_user_lifecycle(self, client: TestClient, sample_user_data: dict):
        ...
        update_response = client.put("/api/v1/users/me", headers=headers, json=update_data)
        ...
        # 6. 通过ID获取用户（如果有此端点）
        by_id_response = client.get(f"/api/v1/users/{user_id}", headers=headers)
        if by_id_response.status_code != status.HTTP_404_NOT_FOUND:  # 如果端点存在
