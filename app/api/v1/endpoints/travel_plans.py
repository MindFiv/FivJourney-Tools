...
async def get_travel_plans(
    ...
    query = query.order_by(desc(TravelPlan.created_at)).offset(skip).limit(limit)
...
async def get_expenses_for_plan(
    ...
    query = (
        select(Expense)
        .where(and_(Expense.travel_plan_id == plan_id, Expense.user_id == current_user.id))
        .offset(skip)
        .limit(limit)
...
async def get_expense_statistics_for_plan(
    ...
    # 获取统计数据
    query = (
        select(Expense.category, func.sum(Expense.amount).label("total_amount"), func.count(Expense.id).label("count"))
        .where(and_(Expense.travel_plan_id == plan_id, Expense.user_id == current_user.id))
    ...
    total_query = select(func.sum(Expense.amount)).where(
        and_(Expense.travel_plan_id == plan_id, Expense.user_id == current_user.id)
    )
